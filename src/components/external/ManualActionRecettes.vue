    ...
    <el-row class="manualactionrecettes-container" style="width: 100%" >
        ...
        <el-col :span=12 style="text-align: right;"><el-button type="primary" @click="sendSensor">Recompilation</el-button></el-col>
            <el-col :span=12 style="text-align: left;">
                <v-icon v-if="computeIcon(this.recetteComputedState)!=''"
                  :color="computeIconColor(this.recetteComputedState)"
                  :name="computeIcon(this.recetteComputedState)" :scale="2" />
                <v-icon v-if="computeIcon(this.recetteComputedLastRun)!=''"
                  :color="computeIconColor(this.recetteComputedLastRun)"
                  :name="computeIcon(this.recetteComputedLastRun)" :scale="2" />
            </el-col>
...
export default {
  ...
  methods: {
    ...
    computeIconColor: function(field) {
      ...
      var url = this.$store.getters.apiurl + "lambdas/8/get_lambda_state?apikey=" + this.$store.getters.creds.token;
      axios
          .post(url, query)
          .then((response) => {
    ...
    getLambdaLastrun(){
      ...
      var query = {'_id': 'r8_notebookslambdasipynb_recalcul_recette'}
      var url = this.$store.getters.apiurl + "lambdas/8/get_lambda_lastrun?apikey=" + this.$store.getters.creds.token;
      axios
          .post(url, query)
          .then((response) => {
