...
export default {
  ...
  methods: {
    ...
    async handleCurrentRecordChange(val) {
      ...
      console.log(val)
      console.log('Coucou0')
      var url = this.$store.getters.apiurl +"getrecord/"+val._index+"/"+val._id+"?token=" + this.$store.getters.creds.token;  
    ...
    getRecord: function(_index, _id){
      ...
      "getrecord/"+_index+"/"+_id+"?token=" + this.$store.getters.creds.token;  
    ...
    loadData: function(download, exportformat) {
      ...
      {
          datacolumns.push(this.config.config.headercolumns[col].field.substring(8));
      }
