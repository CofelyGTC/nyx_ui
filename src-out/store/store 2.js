...
function computeAutoTime(minutes) {
  ...
  else if (minutes <= 360) return '5m'
  else if (minutes <= 1440) return '20m'
  else if (minutes <= 1440 * 2) return '30m'
  else if (minutes <= 1440 * 3) return '2h'
  else if (minutes <= 1440 * 7) return '4h'
  else if (minutes <= 1440 * 32) return '12h'
  else if (minutes <= 1440 * 90) return '1d'
  else if (minutes <= 1440 * 180) return '7d'
  else return '20d'
}
    ...
    check_websocket: (context, payload) => {
      ...
      if (context.getters.wsObject.check_alive) {
        if (context.getters.wsObject.socket != null) {
          if (
            context.getters.wsObject.last_lifesign != null &&
            moment(new Date()) - context.getters.wsObject.last_lifesign > 10000
          ) {
            console.log(
              moment(new Date()) - context.getters.wsObject.last_lifesign
            )
        ...
        if (context.getters.wsObject.socket == null) {
          ...
          var wsurl =
            mainurl.protocol + '//' + mainurl.host + '/nyx_ui_websocket/'

          wsurl = wsurl
            .replace('https://', 'wss://')
            .replace('http://', 'ws://')
          if (context.getters.apiurl.indexOf('http') >= 0) {
            wsurl = context.getters.apiurl
              .replace('http://', 'ws://')
              .replace('https://', 'wss://')
              .replace('/api/v1', '/nyx_ui_websocket/')
          }
  ...
  mutations: {
    ...
    login(state, payload) {
      ...
      state.creds.hasPrivilege = function (inPrivilege) {
        if (_.includes(state.creds.user.privileges, 'admin')) return true
        ...
        if (_.includes(state.creds.user.privileges, inPrivilege)) return true
        return false
      }
      ...
      for (var i in cmenus) {
        if (cmenus[i].category != 'apps') {
          ...
          for (var j in cmenus[i].submenus) {
            cmenus[i].submenus[j].value = cmenus[i].submenus[j].title
              .replace(/ /g, '')
              .toLowerCase()
            cmenus[i].submenus[j].fulltitle =
              cmenus[i].value + '/' + cmenus[i].submenus[j].value
          }
    ...
    refreshTimeRange(state, payload) {
      ...
      if (state.lastTimeRangeEvent.type == 'relative') {
        ...
        state.autoTime = computeAutoTime(
          minutesmulti * state.lastTimeRangeEvent.relativeValue
        )
        state.timeRange = [
          moment().unix() -
            minutesmulti * state.lastTimeRangeEvent.relativeValue * 60,
          moment().unix(),
        ]
      ...
      switch (payload.data.subtype) {
        ...
        default:
          if (payload.data.type == 'absolute') {
            ...
            state.autoTime = computeAutoTime(
              minutesmulti * payload.data.relativeValue
            )
            state.timeRange = [
              moment().unix() - minutesmulti * payload.data.relativeValue * 60,
              moment().unix(),
            ]
            state.timeRange[0] = new Date(state.timeRange[0] * 1000)
            state.timeRange[1] = new Date(state.timeRange[1] * 1000)
    ...
    updatePGRecord(state, payload) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.error != '') console.log('Save PG object error...')
          else {
    ...
    deletePGRecord(state, payload) {
      ...
      var url =
        state.apiurl +
        'pg_generic/' +
        payload.data.config.config.index +
        '/' +
        payload.data.config.config.pkey +
        '/' +
        payload.data.row._id +
        '?token=' +
        state.creds.token
