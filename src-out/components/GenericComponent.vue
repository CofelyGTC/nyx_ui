<template>
  <div
    v-if="$store.getters.currentSubCategory"
    style="border: 0px solid pink; height: 100%; overflow: hidden"
    v-loading="loading"
  >
    <div
      ...
      style="
        ...
        margin-top: 5px;
      "
      v-bind:style="singleStyleContainerComputed"
    >
      <div style="overflow: auto" v-if="!loading">
        <div
          v-if="
            $store.getters.currentSubCategory.apps[0].type == 'generic-table'
          "
        >
        ...
        <div
          v-if="
            $store.getters.currentSubCategory.apps[0].type ==
            'lazy-generic-table'
          "
        >
          <LazyGenericTable
        ...
        <div
          v-else-if="
            $store.getters.currentSubCategory.apps[0].type == 'external'
          "
        >
          <External
            :config="$store.getters.currentSubCategory.apps[0]"
        ...
        <div
          class="kibana"
          v-else-if="$store.getters.currentSubCategory.apps[0].type == 'kibana'"
        >
          <Kibana
            :config="$store.getters.currentSubCategory.apps[0]"
            :directLoad="true"
        ...
        <div
          v-else-if="$store.getters.currentSubCategory.apps[0].type == 'form'"
        >
          <Form :config="$store.getters.currentSubCategory.apps[0]"></Form>
        </div>
        <div
          v-else-if="$store.getters.currentSubCategory.apps[0].type == 'upload'"
        >
        ...
        <div v-else>
          <component
            :config="$store.getters.currentSubCategory.apps[0]"
            v-bind:is="
              $store.getters.currentSubCategory.apps[0].config.controller
            "
          ></component>
        </div>
      </div>
        ...
        >
          ...
          <div style="overflow: auto !important; border: 0px solid pink">
            ...
            <div v-else>
              ...
              <component
                :config="app"
                v-bind:is="app.config.controller"
                :key="app.rec_id"
              ></component>
            </div>
          </div>
