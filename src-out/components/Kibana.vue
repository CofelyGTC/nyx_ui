<template slot="items">
  <div>
    <el-row v-if="config.queryFilterChecked">
      <QueryFilter
        :config="config"
        v-on:queryfilterchanged="queryfilterchanged"
      ></QueryFilter>
    </el-row>
    ...
    <div v-if="urlTooLong()">
      <el-row>
        <el-icon style="font-size: 100px"><el-icon-warning-outline /></el-icon>
      </el-row>

      <el-row>
        <div style="font-size: 20px; margin-top: 10px">
          The URL is too long ({{ computedurl.length }} characters) for your
      ...
      <el-row type="flex" justify="center">
        <el-card
          style="width: 300px; text-align: center; margin-top: 10px"
          shadow="never"
        >
          <el-table :data="browsersData" size="small" style="">
            <el-table-column prop="name" label="Browser" width="180">
            </el-table-column>
            <el-table-column prop="maxSize" label="Max Size"> </el-table-column>
          </el-table>
        </el-card>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
    urlTooLong: function () {
      ...
      if (
        (this.$browserDetect.isEdge || this.$browserDetect.isIE) &&
        this.computedurl.length > 2046
      )
        return true
      if (this.$browserDetect.isChrome && this.computedurl.length > 32778)
        return true
      if (this.$browserDetect.isChromeIOS > 8192) return true
      ...
      if (this.ready) {
        ...
        switch (this.config.timeSelectorType) {
          case 'day':
            ...
            var endTimeAsUtc = moment(this.$store.getters.timeRangeDay[1]).utc()
    ...
    updateQuery(url) {
      ...
      if (
        ...
        if (this.config.config.hiddenQuery.includes('{{user}}')) {
          ...
          this.config.config.hiddenQuery =
            this.config.config.hiddenQuery.replace('{{user}}', user)
        }
        if (this.config.config.hiddenQuery.includes('{{shopid}}')) {
          var shopid = this.$store.getters.creds.user.shopid
          this.config.config.hiddenQuery =
            this.config.config.hiddenQuery.replace('{{shopid}}', shopid)
        }
