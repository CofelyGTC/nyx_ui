    ...
    <el-row>
      <el-col :span="12">
        <el-row>
          <el-card
            shadow="never"
            v-if="activeMQ && activeMQ[0] && activeMQ[0]._source"
          >
            ...
            <el-row>
              <el-col :span="8" style="text-align: center">
                <el-progress
                  ...
                  :status="
                    compute_status(
                      activeMQ[0]._source.memorypercentusage,
                      10,
                      40
                    )
                  "
              ...
              <el-col :span="8" style="text-align: center">
                <el-progress
                  ...
                  :status="
                    compute_status(
                      activeMQ[0]._source.storepercentusage,
                      10,
                      40
                    )
                  "
              ...
              <el-col :span="8" style="text-align: center">
                <el-progress
                  type="dashboard"
                  :status="
                    compute_status(activeMQ[0]._source.temppercentusage, 10, 40)
                  "
        ...
        <el-row>
          <el-card shadow="never" v-if="elastalert">
            ...
            <div
              ...
              :style="
                'background-color:' +
                (elastalert.total == 0 ? '#13CE66' : '#FF4949')
              "
              v-on:click="switch_to('elastalert')"
            >
        ...
        <el-row>
          <el-card shadow="never" v-if="logstash">
            ...
            <div
              ...
              :style="
                'background-color:' +
                (logstash.total == 0 ? '#13CE66' : '#FF4949')
              "
              v-on:click="switch_to('logstash')"
            >
              <div class="alert-number">
        ...
        <el-row>
          <el-card shadow="never" v-if="docker_status && docker_status.records">
            ...
            <div
              ...
              :style="
                'background-color:' +
                (docker_status.records.filter(
                  (rec) => rec._source.status != 'running'
                ).length == 0
                  ? '#13CE66'
                  : '#FF4949')
              "
        ...
        <el-row>
          <el-card shadow="never" v-if="lambdas && lambdas.aggs">
            ...
            <div
              ...
              :style="
                'background-color:' +
                (lambdas.aggs['1'].value == 0 ? '#13CE66' : '#FF4949')
              "
              v-on:click="switch_to('lambdas')"
            >
              <div class="alert-number">
    ...
    <el-row>
      <el-col :span="12">
        <el-card
          shadow="never"
          v-if="
            WebSocketServer && WebSocketServer[0] && WebSocketServer[0]._source
          "
        >
          <div
            slot="header"
            class="clearfix"
            v-on:click="switch_to('users config')"
          >
            <b>Users</b> - Connected:
            <b>{{ WebSocketServer[0]._source.clients[0].clients }}</b>
            - Uniques:
            <b>{{
              Array.from(new Set(WebSocketServer[0]._source.clients[0].logins))
                .length
            }}</b>
          </div>
          <div class="text item" v-on:click="switch_to('users config')">
            <div
              v-for="(item, index) in Array.from(
                new Set(WebSocketServer[0]._source.clients[0].logins)
              )"
              :key="index"
            >
              &nbsp;- {{ item }}
...
export default {
  ...
  methods: {
    ...
    loadData: function () {
      ...
      if (this.$store.getters.elasticVersion == 6) url += '&doc_type=elastalert'
