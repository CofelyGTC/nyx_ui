    ...
    <el-row class="manualactionsulg-container" style="width: 100%">
      ...
      <el-row>
        ...
        <el-col :span="12" style="text-align: left">
          <v-icon
            v-if="computeIcon(this.ulgComputedState) != ''"
            :color="computeIconColor(this.ulgComputedState)"
            :name="computeIcon(this.ulgComputedState)"
            :scale="2"
          />
          <v-icon
            v-if="computeIcon(this.ulgComputedLastRun) != ''"
            :color="computeIconColor(this.ulgComputedLastRun)"
            :name="computeIcon(this.ulgComputedLastRun)"
            :scale="2"
          />
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    getLambdaState() {
      ...
      var url =
        this.$store.getters.apiurl +
        'lambdas/2/get_lambda_state?apikey=' +
        this.$store.getters.creds.token
    ...
    getLambdaLastrun() {
      ...
      var url =
        this.$store.getters.apiurl +
        'lambdas/2/get_lambda_lastrun?apikey=' +
        this.$store.getters.creds.token
      axios.post(url, query).then((response) => {
