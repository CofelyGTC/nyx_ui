    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span style="font-weight: bold"
          >KPI600 - Administratieve Vergaderingen</span
        >
      </div>
        ...
        <span v-if="monthSelected">
          <el-row :span="24" class="parameters-selection">
            <el-col :span="12" style="text-align: left">
              ...
              <el-row>
                <el-select
                  v-if="selectedLot == 3 || selectedLot == 2"
                  v-model="selectedTec"
            ...
            >
              ...
              >
                <el-switch
                  ...
                  @change="kpiChanged()"
                ></el-switch>
                <span class="kpi600-switches-text">{{
                  kpi600Model.kpi601.text
                }}</span>
              </el-form-item>
              <el-form-item
              ...
              >
                <el-switch
                  ...
                  @change="cancelByCustomerChanged()"
                ></el-switch>
                <span class="kpi600-switches-text">{{
                  kpi600Model.cancelByCustomer.text
                }}</span>
              </el-form-item>
            </el-form>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    dateSelected() {
      ...
      var limit = 14
      if (this.config.config.controllerparameters == 'DNB') limit = 16
      ...
      this.strPeriod1 =
        'Vergadering van ' +
        moment(this.monthSelected).subtract(1, 'months').format('21 MMM YYYY') +
        ' to ' +
        moment(this.monthSelected).format('14 MMM YYYY')
      this.strPeriod2 =
        'Overleg KPI van ' +
        moment(this.monthSelected)
          .startOf('Month')
          .subtract(2, 'months')
          .format('MMM YYYY')
      this.getData()
    ...
    getData() {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.error != '') console.log('get KPI600 monthly error')
          else {
            ...
            this.kpi600Model.cancelByCustomer.value =
              this.currentKPI600MonthlyObj.cancel_by_customer
          }
        })
        .catch((error) => {
          console.log(error)
          ...
          if (response.data.error != '')
            ...
            for (var i in entityModel0) {
              ...
              if (this.config.config.controllerparameters == 'DNB') {
                if (i == '4') this.entityModel[i] = entityModel0[i]
              } else {
                if (i != '4') this.entityModel[i] = entityModel0[i]
              }
            }
