      ...
      <el-card class="box-card2">
        <div slot="header" class="clearfix">
          <span style="font-weight: bold"
            >KPI603 Vergadering: uitvoeren van specifieke actiepunten op basis
            van afgesproken deadlines (behave projecten)</span
          >
        </div>
        <div class="card-body">
  ...
  methods: {
    ...
    loadData() {
      ...
      axios
        .post(url, fullq)
        .then((response) => {
          if (response.data.error != '') console.log('KPI603 list error...')
          else {
