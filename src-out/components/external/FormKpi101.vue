  ...
  <el-row class="kpi101-container" :span="24" type="flex" justify="center">
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="40%">
      <el-form
        ...
        ref="formCall"
      >
        <el-form-item label="CM/EQ Nummer " :label-width="formLabelWidth">
          <el-input-number
        ...
        <el-form-item label="Datum uur " :label-width="formLabelWidth">
          <el-date-picker
            :shortcuts="
        ...
        <el-form-item label="Group " :label-width="formLabelWidth">
          <div>
            <el-radio-group v-model="dialogObj.lot" size="small">
              <el-radio-button
                v-for="item in lotModel"
                :key="item.value"
                :label="item.label"
      ...
      <div class="card-body">
        ...
        <el-row class="fourth-row" :span="24">
          ...
          >
            ...
            <el-table-column sortable prop="lot" label="Lot" width="100">
            </el-table-column>
            <el-table-column sortable prop="date" label="Datum" width="170">
            </el-table-column>
            <el-table-column sortable prop="hour" label="Uur" width="170">
            </el-table-column>
            <!--<el-table-column
                  sortable
                  prop="phoneNumber"
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          if (response.data.error != '')
            ...
            for (var i in response.data.records) {
              response.data.records[i]._source._id =
                response.data.records[i]._id
              response.data.records[i]._source.date = moment(
                response.data.records[i]._source.datetime
              ).format('DD/MM/YY')
              response.data.records[i]._source.hour = moment(
                response.data.records[i]._source.datetime
              ).format('hh:mm')

              for (var j in this.lotModel) {
                if (
                  this.lotModel[j].value == response.data.records[i]._source.lot
                )
                  response.data.records[i]._source.label =
                    this.lotModel[j].label
              }
            ...
            for (var i in this.lotModel) {
              ...
              try {
                lotModel.notTimelyAnswered = _.filter(this.callData, {
                  lot: lotModel.value,
                }).length
              } catch {
              ...
              .then((response) => {
                if (response.data.error != '')
                  ...
                  try {
                    ...
                    for (var i in this.lotModel) {
                      this.lotModel[i].percentage =
                        res._source['percentage_' + this.lotModel[i].value]

                      if (
                        res._source[
                          'number_of_call_' + this.lotModel[i].value
                        ] != null
                      ) {
                        this.lotModel[i].numberOfCall =
                          res._source[
                            'number_of_call_' + this.lotModel[i].value
                          ]
                      } else {
                ...
                try {
                  for (var i in this.lotModel) {
                    var lotModel = this.lotModel[i]
                    if (lotModel.numberOfCall == 0) lotModel.percentage = 0
                    else {
                      var percent =
                        100 *
                        ((lotModel.numberOfCall - lotModel.notTimelyAnswered) /
                          lotModel.numberOfCall)
    ...
    handleChangeNumberCall: _.debounce(function () {
      ...
      for (var i in this.lotModel) {
        obj['number_of_call_' + this.lotModel[i].value] =
          this.lotModel[i].numberOfCall
      }
          ...
          if (response.data.error != '')
            ...
            try {
              for (var i in this.lotModel) {
                ...
                lotModel.notTimelyAnswered =
                  res['not_timely_answer_' + lotModel.value]
