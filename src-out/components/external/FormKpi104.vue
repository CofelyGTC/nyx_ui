  ...
  <el-row class="kpi104-container" :span="24" type="flex" justify="center">
    <el-card class="box-card">
      ...
      <div class="card-body">
        ...
        <el-row class="second-row" :span="24"> </el-row>
        <span v-if="weekSelected">
          <el-row class="header1" :span="24">
            <el-col :offset="6" :span="6" style="font-weight: bold"
              >6u - 14u</el-col
            >
...
export default {
  ...
  methods: {
    ...
    detectFirstLastDate() {
      ...
      this.lastDate = moment(this.weekSelected)
        .day('Monday')
        .startOf('Day')
        .add(6, 'days')
    },
    dateSelected() {
      ...
      for (var i in this.dayConversion) {
        var date = moment(this.weekSelected)
          .day('Monday')
          .startOf('Day')
          .add(count, 'days')
          .format('DD MMM YYYY')
        this.dayConversion[i] =
          i +
          ' - ' +
          moment(this.weekSelected)
            .day('Monday')
            .startOf('Day')
            .add(count, 'days')
            .format('DD/MM/YY')
      ...
      if (moment().format('D') >= 14)
        this.disable =
          moment() > moment(this.weekSelected).endOf('Week').endOf('Month')
      else
        this.disable =
          moment().subtract(1, 'months') >
          moment(this.weekSelected).endOf('Week').endOf('Month')

      if (!this.writeAccess) this.disable = true
    ...
    getData() {
      ...
      var end = moment(this.weekSelected)
        .day('Sunday')
        .endOf('day')
        .add(7, 'days')
      ...
      axios
        .post(url, query)
        .then((response) => {
          if (response.data.error != '') console.log('KPI104 get data error...')
          else {
            ...
            for (var i in response.data.records) {
              ...
              if (rec.value) this.flagTrue = true
              else this.flagFalse = true
            }
    ...
    determineIsIndeterminated: function () {
      if (this.flagFalse && this.flagTrue) {
        ...
        this.isIndeterminate = false
        if (this.flagFalse) this.checkAll = false
    ...
    switchChange: _.debounce(function () {
      ...
      var count = 0
      var startDate = moment(this.weekSelected)
        .day('Monday')
        .startOf('Day')
        .add(count, 'days')
      var lastDate = moment(this.weekSelected)
        .day('Monday')
        .startOf('Day')
        .add(count, 'days')
      ...
      for (var i in this.switchModel) {
        ...
        for (var j in this.switchModel[i]) {
          ...
          if (j % 2 == 0) obj.ronde = 'rondier1'
          else obj.ronde = 'rondier2'

          if (j < 2) obj.time = '6u - 14u'
          else if (j < 4) obj.time = '14u - 22u'
          else obj.time = '22u - 6u'
      ...
      setTimeout(() => {
        axios
          ...
          .then((response) => {
            ...
            else console.log('KPI104 update monthly success')
          })
          .catch((error) => {
            console.log(error)
          })
