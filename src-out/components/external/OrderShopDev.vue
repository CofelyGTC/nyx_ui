<template>
  <div style="width: 100%">
    <el-row class="ordershopdev-container" style="width: 100%">
      <el-form style="widht: 100%" :disabled="this.disabled || this.isCloture">
        <el-row>
          <td>
            Clôturer la commande :
            <el-switch @change="onCloseOrder()" v-model="disabled"> </el-switch>
          </td>
        </el-row>
        <el-tabs v-model="selectedTab" @tab-click="tabChanged(selectedTab)">
            ...
            <el-tabs v-model="selectedUnderTab" @tab-click="subTabChanged()">
              ...
              >
                <el-row>
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl3']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter1" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id1) in subSubCategories[category][
                          subcategory
                        ]['sortLvl3']"
                        :key="id1"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl4']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter2" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id2) in subSubCategories[category][
                          subcategory
                        ]['sortLvl4']"
                        :key="id2"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl5']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter3" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id3) in subSubCategories[category][
                          subcategory
                        ]['sortLvl5']"
                        :key="id3"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl6']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter4" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id4) in subSubCategories[category][
                          subcategory
                        ]['sortLvl6']"
                        :key="id4"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl7']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter5" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id5) in subSubCategories[category][
                          subcategory
                        ]['sortLvl7']"
                        :key="id5"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl8']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter6" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id6) in subSubCategories[category][
                          subcategory
                        ]['sortLvl8']"
                        :key="id6"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl9']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter7" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id7) in subSubCategories[category][
                          subcategory
                        ]['sortLvl9']"
                        :key="id7"
                        :label="item"
                  ...
                  <el-col
                    v-if="
                      subSubCategories[category][subcategory]['sortLvl10']
                        .length > 1
                    "
                    :span="4"
                  >
                    ...
                    <el-select v-model="filter8" placeholder="Sélectionner">
                      <el-option
                        v-for="(item, id8) in subSubCategories[category][
                          subcategory
                        ]['sortLvl10']"
                        :key="id8"
                        :label="item"
                ...
                <div style="bottom: 5%">
                  <el-row
                    >Total Boulangerie: {{ totalBoulangerie | roundTo2 }}€ Total
                    Pâtisserie TTC : {{ totalPatisserie | roundTo2 }} €
                  </el-row>
                  <el-row>
                    Total Salés TTC : {{ totalSales | roundTo2 }} € Autres TTC :
                    {{ totalOther | roundTo2 }} €</el-row
                  >

                  <el-row
                    >Total Sélection: {{ totalFiltered | roundTo2 }}€ Total
                    Panier TTC : {{ totalPrice | roundTo2 }} €</el-row
                  >
                  <!--el-table :data="records.filter(data => data.sortLvl1 == category && data.sortLvl2 == subcategory && data.sortLvl3 == filter1)" style="width: 100%"-->
                  <el-table
                    :data="records.filter(getFilter)"
                    style="
                      width: 100%;
          ...
          >
            ...
            >
              Sous-Total :
              <el-row v-for="(category, index) in classement" :key="index">
                <h1>
                  {{ category }}: Quantité totale:
                  {{ totalCategoryQuantity(category) | roundTo2 }} Total TTC:
                  {{ totalCategoryPrice(category) | roundTo2 }}€
                </h1>
                <el-row
                  v-for="(subCategory, index1) in subCategories[category]"
                  :key="index1"
                >
                  {{ subCategory }}: Quantité totale:
                  {{
                    totalSubCategoryQuantity(category, subCategory) | roundTo2
                  }}
                  Total TTC:
                  {{ totalSubCategoryPrice(category, subCategory) | roundTo2 }}€
                </el-row>
              </el-row>
              <el-row> Total Panier: {{ totalPrice | roundTo2 }}€ TTC </el-row>
              <el-row>
                <br /><br />
  ...
  computed: {
    ...
    totalSales: function () {
      ...
      for (var itemKey in Object.keys(this.records)) {
        ...
        if (
          data.sortLvl1 == 'Salés' &&
          data.sortLvl2 != 'Quiches' &&
          data.sortLvl2 != 'Divers'
        ) {
          price += data.conditionnement * data.quantity * data.Prix_TVAC
        }
      }
    ...
    totalOther: function () {
      ...
      for (var itemKey in Object.keys(this.records)) {
        ...
        if (
          data.sortLvl2 == 'Quiches' ||
          (data.sortLvl2 == 'Divers' && data.sortLvl1 == 'Salés') ||
          (data.sortLvl1 != 'Pâtisserie' &&
            data.sortLvl1 != 'Boulangerie' &&
            data.sortLvl1 != 'Salés')
        ) {
          price += data.conditionnement * data.quantity * data.Prix_TVAC
        }
    ...
    totalFiltered: function () {
      ...
      for (var itemKey in Object.keys(this.records)) {
        var data = this.records[itemKey]
        var filter =
          data.sortLvl1 == this.selectedCategory &&
  ...
  methods: {
    ...
    addLog(order) {
      ...
      var newLog = {
        ...
        boulangerie: this.totalBoulangerie,
        patisserie: this.totalPatisserie,
        sales: this.totalSales,
        other: this.totalOther,
      }
              ...
              if (response.data.error != '') {
                this.$notify({
                  ...
                  message:
                    'Commande en ' +
                    this.categoryUp +
                    ' a echoué, veuillez recharger la page et réessayer',
                  type: 'error',
    ...
    getTree() {
      var url =
        this.$store.getters.apiurl +
        'schamps/get_products_tree?token=' +
        this.$store.getters.creds.token
    ...
    createNewForm() {
      ...
      var url =
        this.$store.getters.apiurl +
        'lambdas/8/get_products_list_by_shop?apikey=' +
        this.$store.getters.creds.token
      ...
      axios.post(url, query).then((response) => {
        if (response.data.error != '') {
          ...
          for (var i in response.data.products) {
            response.data.products[i]._source._id =
              response.data.products[i]._id
            response.data.products[i]._source.quantity = 0
            response.data.products[i]._source.orderquantity = 0
            console.log(
              'Retrieved data : ' + JSON.parse(JSON.stringify(response.data))
            )
            ...
            if (response.data.products[i]._source.avail == 'always') {
              ...
              var period = JSON.parse(
                response.data.products[i]._source.availabilityConf
              )
              ...
              if (
                timeRange[0].getTime() >= start &&
                timeRange[0].getTime() <= stop
              ) {
                this.callData.push(response.data.products[i]._source)
              } else {
    ...
    getCloture() {
      var query = {
        mes: 'coucou',
        date: this.$store.getters.timeRangeDay[0].getTime(),
      }
      var url =
        this.$store.getters.apiurl +
        'lambdas/8/get_cloture?apikey=' +
        this.$store.getters.creds.token
      axios.post(url, query).then((response) => {
        var status = false
          ...
          axios
            .post(
              this.$store.getters.apiurl +
                ...
                this.$store.getters.creds.token,
              {
                month_to_update: moment(this.monthSelected).format(
                  'YYYY-MM-DD'
                ),
              }
            )
