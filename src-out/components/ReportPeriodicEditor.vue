<template>
  <el-dialog
    width="80%"
    :title="$t('scheduler.edit_scheduler')"
  ...
  >
      ...
      <el-tab-pane :label="$t('scheduler.scheduler')" name="scheduler">
        ...
        >
          ...
          <el-row>
            ...
            <el-col :span="2">
              <v-icon
                v-if="record._source.icon"
                :name="record._source.icon"
                scale="2"
              />
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item
                :label="$t('scheduler.path')"
                :label-width="formLabelWidth"
              >
                <el-input
                  size="small"
                  placeholder="The path + filename  on the server used to save the report. Empty by default."
                  v-model="record._source.path"
                  autocomplete="off"
                ></el-input>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="18">
              <template v-if="record._source.trigger.type == 'daily'">
                <el-transfer
                  ...
                  :titles="[
                    $t('scheduler.excluded_days'),
                    $t('scheduler.included_days'),
                  ]"
                  filterable
      ...
      <el-tab-pane :label="$t('scheduler.reset_task')" name="reset task">
        <el-form>
          <el-row>
            <el-col :span="6">
              <el-form-item
                :label="$t('scheduler.number_of_days')"
                :label-width="formLabelWidth"
              >
                <el-input-number
                  :min="1"
        ...
        <el-form>
          <el-row>
            ...
            >
              <el-input
                size="small"
                v-model="record._source.mailSubject"
                autocomplete="off"
              ></el-input>
            </el-form-item>
          </el-row>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button
        type="primary"
        @click="updatePeriodicReport()"
        :disabled="!modified"
        >{{ $t('buttons.update') }}</el-button
      >
    </span>
  </el-dialog>
</template>
    ...
    rangePickerOptions: {
      shortcuts: [
        ...
        },
        {
          text: 'Today',
          onClick(picker) {
            const end = moment().startOf('day').add(1, 'd').toDate()
