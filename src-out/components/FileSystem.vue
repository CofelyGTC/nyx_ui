        ...
        <el-row style="height: 28px" flex>
          <el-col :span="16">
            ...
            >
              <el-breadcrumb-item
                class="noselect"
                :to="{ path: rootRouterPath }"
                >{{ config.config.foldername }}</el-breadcrumb-item
              >
              <el-breadcrumb-item
                class="noselect"
      ...
      >
          ...
          <template slot-scope="scope" style="font-weight: bold">
            <span class="icon-items">
              ...
              <v-icon
                v-else-if="
                  scope.row.type == 'file' &&
                  ['png', 'jpg'].indexOf(scope.row.extension.toLowerCase()) > -1
                "
                name="regular/image"
                scale="1.3"
              />
              <v-icon
                ...
                v-else-if="
                  scope.row.type == 'file' &&
                  ['pdf'].indexOf(scope.row.extension.toLowerCase()) > -1
                "
                name="file-pdf"
                scale="1.3"
              />
              <v-icon
                ...
                v-else-if="
                  scope.row.type == 'file' &&
                  ['doc', 'docx'].indexOf(scope.row.extension.toLowerCase()) >
                    -1
                "
                name="file-word"
                scale="1.3"
              ...
              <v-icon
                ...
                v-else-if="
                  scope.row.type == 'file' &&
                  ['xlsx', 'xls', 'xlsm', 'xltx', 'xltm'].indexOf(
                    scope.row.extension.toLowerCase()
                  ) > -1
                "
                name="file-excel"
                scale="1.3"
              ...
              <v-icon
                ...
                v-else-if="
                  scope.row.type == 'file' &&
                  ['csv'].indexOf(scope.row.extension.toLowerCase()) > -1
                "
                name="file-csv"
                scale="1.3"
              />
              <v-icon
                ...
                v-else-if="
                  scope.row.type == 'file' &&
                  ['ppt', 'pptx'].indexOf(scope.row.extension.toLowerCase()) >
                    -1
                "
                name="regular/file-powerpoint"
                scale="1.3"
              />
              <v-icon
                v-else-if="
                  scope.row.type == 'file' &&
                  ['rtf', 'txt'].indexOf(scope.row.extension.toLowerCase()) > -1
                "
                name="regular/file-alt"
                scale="1.3"
              />
        ...
        >
          <template slot-scope="scope"
            ><span class="noselect">{{
              scope.row.modification_time | formatTS
            }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="size" sortable label="File size" width="150">
          <template slot-scope="scope"
            ><span class="noselect" v-if="scope.row.type == 'file'">{{
  ...
  watch: {
    $route(to, from) {
      if (from.path === to.path && to.params.recid === this.config.rec_id) {
        console.log(
          'Navigation into the same file system ' + this.config.rec_id
        )
        console.log(from)
        console.log(to)
  ...
  methods: {
      ...
      setTimeout(() => {
        if (this.numberOfItems == 0) {
          ...
          this.tableData = this.tableData.concat(
            this.allData.splice(0, this.scrollSize)
          )
          $state.loaded()
        }
      }, 50)
    ...
    rowClick(row, column, event) {
      if (this.$refs.filesTable.selection.includes(row))
        ...
        if (!event.shiftKey) this.$refs.filesTable.clearSelection()
        ...
        if (this.config.config.editors != null) {
          for (const editorConf of this.config.config.editors) {
            ...
            if (row.name.indexOf(extension) > 0) {
              console.log(
                'open the file with this editor : ' + editorConf.editorName
              )
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.error == '') {
            ...
            this.allData = response.data.data.sort((a, b) =>
