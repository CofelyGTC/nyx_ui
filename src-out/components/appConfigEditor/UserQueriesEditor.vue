  ...
  >
    <el-card shadow="never">
      ...
      <el-row v-if="currentConfig.queryFilterChecked">
        <el-button @click="setFocus('fieldsToFilter')" type="text"
          >Fields to filter</el-button
        >
      </el-row>
      <el-row v-if="currentConfig.queryFilterChecked">
        <el-col
        ...
        <el-col :span="12" class="padding-right">
          <el-card shadow="never">
            <table class="table-display">
              ...
              >
                <tr
                  v-for="(item, index) in currentConfig.config.queryfilters"
                  :key="index"
                >
                  <td>{{ item.field }}</td>
                  <td>
                    <el-button @click="configureQueryFilter(item)" size="small"
                      >Configure</el-button
                    >
                  </td>
            ...
            <div style="width: 100%; text-align: center; margin-top: 10px">
              <el-button
                ...
                size="small"
                v-if="fieldsToFilter.indexOf('Global_Filter') < 0"
                >Add Global Filter</el-button
              >
            </div>
          </el-card>
        </el-col>
        <el-col :span="12" v-if="filterFieldToConfigure">
          <el-card shadow="never">
            <el-row>
              <el-col :span="12" class="padding-right">
                <el-row>
                  <el-button @click="setFocus('label')" type="text"
                    >Label</el-button
                  >
                </el-row>
                <el-row>
              ...
              <el-col :span="12">
                <el-row>
                  <el-button @click="setFocus('default')" type="text"
                    >Default</el-button
                  >
                </el-row>
                <el-row>
            ...
            <el-row>
              <el-col
                :span="12"
                v-if="filterFieldToConfigure.type != 'globaltext'"
              >
                <el-row>
                  <el-button @click="setFocus('type')" type="text"
                    >Type</el-button
                  >
                </el-row>
                <el-row>
...
export default {
  ...
  computed: {
    ...
    filterFieldToConfigureID() {
      if (this.filterFieldToConfigure == null) return null
      ...
      handler: function () {
        if (this.filterFieldToConfigure.selectOptions != null)
          this.selecterOptions =
            this.filterFieldToConfigure.selectOptions.join('\r\n')
        else this.selecterOptions = ''
      },
    },
    selecterOptions: {
      handler: function () {
  ...
  methods: {
    ...
    addGlobalFilter: function () {
      this.currentConfig.config.queryfilters.push({
        field: 'Global_Filter',
        column: 'global',
        title: 'Global',
        type: 'globaltext',
    ...
    selecterOptionsToConfig: _.debounce(function () {
      this.filterFieldToConfigure.selectOptions = this.selecterOptions
        .trim()
        .replace(/\r/g, '')
        .split('\n')
    }, 500),
    ...
    selectFieldsToFilterChanged: function (val) {
      let tmp = JSON.parse(
        JSON.stringify(this.currentConfig.config.queryfilters)
      )
      this.currentConfig.config.queryfilters = []
    ...
    handleClose: function (tag) {
      this.selecterOptions.splice(this.selecterOptions.indexOf(tag), 1)
    },
    handleInputConfirm: function () {
      let inputValue = this.inputValue
      if (this.selecterOptions == undefined) this.selecterOptions = []
      if (inputValue) {
