<template>
  ...
  <draggable
   ...
   -->
    ...
    <el-button round type="primary" @click="addView" class="add-view-button">
      Add a view
    </el-button>
...
export default {
  ...
  computed: {
    ...
    newrecgroupchanged: function () {
      return (
        JSON.stringify(this.recordin) != JSON.stringify(this.newrecfromgroup)
      )
    },
  ...
  methods: {
    ...
    saveRecord: function () {
      ...
      for (var i in this.newrecfromgroup) {
        ...
        if (this.newrecfromgroup[i].group == null) obj.group = 0
        else obj.group = parseInt(this.newrecfromgroup[i].group)
            ...
            for (var i in this.orgRec._source.id_array) {
              for (var j in this.viewListAll) {
                ...
                if (this.orgRec._source.id_array[i].id == viewRec.id) {
                  ...
                  this.viewListGroup[viewRec.group].push(
                    JSON.parse(JSON.stringify(viewRec))
                  )
                }
              }
            }
