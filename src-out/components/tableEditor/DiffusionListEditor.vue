<template>
  ...
  >
    <el-row style="text-align: left">
      <el-button @click="setFocus('author')" type="text">Author</el-button>
    </el-row>
    ...
    <el-row style="text-align: left">
      <el-button @click="setFocus('recipientList')" type="text"
        >Recipient List</el-button
      >
    </el-row>
    ...
    <el-row style="text-align: left">
      <el-card shadow="hover">
        <table class="table-entities">
          ...
          <draggable
            v-bind="dragOptions"
            v-model="newRec._source.header.list"
            tag="tbody"
            handle=".handle"
          >
            <tr
              v-for="(item, index) in newRec._source.header.list"
              :key="index"
            >
...
export default {
  ...
  data: () => ({
    ...
    viewToModify: null,
    dragging: false,
    companyList: ['Cofely', 'Brussels Airport Company'],
  }),
  computed: {
    recordin: function () {
      return this.record
  ...
  methods: {
    ...
    prepareData: function () {
      ...
      for (var i = 0; i < this.orgRec._source.header.list.length; i++) {
        ...
        if (!this.companyList.includes(company)) this.companyList.push(company)
      }
