<template>
  ...
  >
    <el-row
      ><h1>{{ newRec._source.shop }}</h1></el-row
    >

    <el-row
      >Total Boulangerie: {{ totalBoulangerie | roundTo2 }}€ Total Pâtisserie
      TTC : {{ totalPatisserie | roundTo2 }} €
    </el-row>
    <el-row>
              ...
              <el-tabs v-model="selectedUnderTab" @tab-click="subTabChanged()">
                ...
                >
                  ...
                  <div style="bottom: 5%">
                    ...
                    Panier TTC : {{ totalPrice | roundTo2 }} €

                    <el-table
                      :data="newRec._source.products.filter(getFilter)"
                      style="
                        width: 100%;
  ...
  methods: {
    ...
    addProduct: function () {
      ...
      axios.post(url, query).then((response) => {
        ...
        if (
          !this.subCategories[response.data.sortLvl1].includes(
            response.data.sortLvl2
          )
        ) {
          this.subCategories[response.data.sortLvl1].push(
            response.data.sortLvl2
          )
        }
        ...
        if (response.data.error != '')
          ...
          for (var index2 in classement) {
            ...
            for (var index in this.newRec._source.products) {
              ...
              if (
                this.newRec._source.products[index]['sortLvl1'] == cat &&
                this.newRec._source.products[index]['display'] == true
              ) {
          ...
          for (var index in classement) {
            ...
            if (classement[index] in tree) {
              ...
              for (var sub in subCategories[classement[index]]) {
                //console.log(sub)
                tree[classement[index]][subCategories[classement[index]][sub]] =
                  {
                    sortLvl3: ['-'],
                    sortLvl4: ['-'],
                    sortLvl5: ['-'],
            ...
            for (var index2 in subCategories[classement[index]]) {
              if (subCategories[classement[index]] in tree[classement[index]]) {
                ...
                tree[classement[index]][
                  subCategories[classement[index]][index2]
                ] = {
                  sortLvl3: ['-'],
                  sortLvl4: ['-'],
                  sortLvl5: ['-'],
