<template>
  <svg :viewBox="viewBox2" :width="containerWidth">
    ...
    <rect
      v-show="dragStarted"
      :x="
        movedragXRef - startdragXRef > 0
          ? startdragXRef + marginleft
          : startdragXRef + marginleft + (movedragXRef - startdragXRef)
      "
      :y="margintop"
      :width="
        movedragXRef - startdragXRef > 0
          ? Math.max(5, movedragXRef - startdragXRef)
          : Math.max(5, startdragXRef - movedragXRef)
      "
      pointer-events="none"
      :height="height - margintop - marginbottom"
    ...
    <text :x="marginleft - 10" :y="margintop + 8" text-anchor="end">
      {{ max }}
    </text>
    <text :x="marginleft - 10" :y="height - marginbottom" text-anchor="end">
      0
    </text>
    <text
      :x="marginleft / 2"
...
export default {
  ...
  methods: {
    ...
    backgroundUp: function (evt) {
      if (this.dragStarted) this.backUp(evt)
    },
    backUp: function (evt) {
      if (
        !(
      ...
      var startt = moment(this.$store.getters.timeRange[0]).add(staX, 'seconds')
