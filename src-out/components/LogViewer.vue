      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag
              size="small"
              v-if="scope.row.severity.toLowerCase() == 'error'"
              type="danger"
              effect="dark"
            ...
            <el-tag
              size="small"
              v-else-if="scope.row.severity.toLowerCase() == 'warning'"
              type="warning"
              effect="dark"
              >{{ scope.row.severity }}</el-tag
            >
            <el-tag
              size="small"
              v-else-if="scope.row.severity.toLowerCase() == 'info'"
              type=""
              effect="plain"
              >{{ scope.row.severity }}</el-tag
            >
            <el-tag size="small" v-else type="info" effect="dark">{{
              scope.row.severity
            }}</el-tag>
          </template>
        </el-table-column>
        ...
        .then((response) => {
          if (response.data.error != '') {
            ...
            for (var i = 0; i < response.data.data._source.logs.length; i++) {
              ...
              var obj = {
                date: moment(log[0], 'DDMMMYYYY HH:mm:ss:SSS').format(
                  'HH:mm:ss'
                ),
                severity: log[1],
