<template>
  <div>
    ...
    <span v-if="dialogFormVisible">
      <component
        v-if="
          config.config.editorComponent != null &&
          config.config.editorComponent != ''
        "
      ...
      <el-col :span="24">
        ...
        >
          <el-table-column
            ...
            :key="header.field"
            :label="
              computeTranslatedText(
                header.title,
                $store.getters.creds.user.language
              )
            "
            :prop="header.field"
            ...
            <template slot-scope="scope">
              <span v-if="header.type == 'select'">
                <el-select
                  ...
                  size="small"
                  v-model="
                    scope.row._source[header.field.replace('_source.', '')]
                  "
                  placeholder="Select"
                >
                  <el-option
              ...
              <span
                v-else-if="
                  (header.type == 'long' || header.type == 'double') &&
                  header.format != null &&
                  header.format.length > 0
                "
                >{{
            ...
            <template slot="header" slot-scope="scope">
              <div>
                <el-tooltip
                  :show-after="1000"
                  v-if="
                    config.config.index.indexOf('*') == -1 &&
                    currentRow &&
                    $store.getters.creds.hasPrivilege(
                      config.config.writeprivileges
                    )
                  "
                  class="item"
                  effect="light"
  ...
  methods: {
    sortChanged: function (e) {
      ...
      if (e.column.order != null)
        this.sort[e.column.property.replace('_source.', '')] = {
          order: e.column.order.substring(0, 4).replace('asce', 'asc'),
        }
    ...
    async handleCurrentRecordChange(val) {
      ...
      //var rec = await this.getRecord(val._index, val._id)
      var rec = await axios.get(url)
      console.log('Coucou2')
      console.log(rec)
      this.currentRow = rec.data.data
      this.currentRow.original = rec.data.data
      console.log(val)
    ...
    async getRecordFromRow(row) {
      try {
        ...
        if (
          ...
          console.warn('fail to retrieve the document, returning the row')
          return row
        }
      } catch (e) {
        console.error(e)
    ...
    loadData: function (download, exportformat) {
      ...
      for (var col in this.config.config.headercolumns) {
        datacolumns.push(
          this.config.config.headercolumns[col].field.substring(8)
        )
      }
      ...
      if (
        ...
        if (this.config.config.hiddenQuery.includes('{{user}}')) {
          this.config.config.hiddenQuery =
            this.config.config.hiddenQuery.replace(
              '{{user}}',
              this.$store.getters.creds.user.id
            )
        }
        ...
        if (this.config.config.hiddenQuery.includes('{{shopid}}')) {
          ...
          if (this.shopID != null) {
            ...
            this.config.config.hiddenQuery =
              this.config.config.hiddenQuery.replace('{{shopid}}', this.shopID)
          }
          console.log('Hidden Query : ' + this.config.config.hiddenQuery)
        }
      ...
      if (query.extra == undefined)
        query.extra = { currentpage: this.currentPage, pagesize: this.pagesize }
      else {
        query.extra.currentpage = this.currentPage
        query.extra.pagesize = this.pagesize
      }

      query._source = datacolumns
      ...
      if (this.config.mapChecked && !this.dontrefreshMap) {
        ...
        if (
          this.config.config.functionfields != null &&
          this.config.config.functionfields != ''
        ) {
        ...
        axios.post(url, query2).then((response) => {
    ...
    setAutoRefresh: function () {
      if (this.config.autoRefreshTime == null) return
