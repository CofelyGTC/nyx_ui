<template>
  ...
  <div>
    ...
    >
      ...
      <el-collapse>
        <el-collapse-item
          :title="$t('report.details_on_run') + selectedReportInfos.id"
          name="1"
        >
          <el-table
            :data="tableInfo"
    ...
    <el-table
      ...
      :data="
        tableData.filter(
          (data) =>
            !search ||
            data._source.creds.user.id
              .toLowerCase()
              .includes(search.toLowerCase()) ||
            data._source.report.title
              .toLowerCase()
              .includes(search.toLowerCase())
        )
      "
      :default-sort="{
        prop: '_source.treatment.creation',
        order: 'descending',
      }"
      style="width: 100%"
      height="1000"
    ...
    >
      ...
      <el-table-column
        sortable
        prop="_source.treatment.creation"
        :label="$t('generic.creation')"
      >
        <template slot-scope="scope">
          <div>{{ scope.row._source.treatment.creation | formatDate }}</div>
          <span v-if="scope.row._source.treatment.duration"
            >Duration
            {{
              scope.row._source.treatment.duration | numeral('0.[0]')
            }}
            s</span
          >
        </template>
          ...
          <div style="font-weight: bolder">
            {{
              computeTranslatedText(
                scope.row._source.report.title,
      ...
      <el-table-column
        ...
        prop="_source.creds.user.id"
        sortable
      >
        <template slot-scope="scope">
          <div style="font-weight: bolder">
            {{ scope.row._source.creds.user.id }}
          </div>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('generic.parameters')"
        prop="_source.report.parameters.length"
      >
        <template slot-scope="scope">
          <el-popover
          ...
          >
            ...
            <el-button size="small" slot="reference">{{
              scope.row._source.report.parameters.length
            }}</el-button>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column align="right">
        <template slot-scope="scope">
          <el-tooltip
            ...
            placement="bottom"
            v-if="
              scope.row._source.report.reportType == 'notebook' ||
              scope.row._source.report.reportType == 'notebook_doc'
            "
          >
            <el-button
